# kafka
Playbooks/Roles used to deploy Kafka; supports deployments of either the [Confluent](https://www.confluent.io/) or [Apache](https://kafka.apache.org/) Kafka distributions.
...
```bash
$ git clone https://github.com/Datanexus/kafka
```

That command will pull down the repository, which includes the [provision-kafka.yml](provision-kafka.yml) playbook and all of its dependencies.
...
We discuss the various deployment scenarios supported by this playbook in [this document](docs/Deployment-Scenarios.md). With no changes to the the contents of repository (more on this below), the [provision-kafka.yml](provision-kafka.yml) playbook can be used (out of the box, so to speak) to deploy a three node Kafka cluster to an AWS environment using a command that looks something like this:
...
* It avoids modifying files that are being tracked under version control in the main, [kafka](https://github.com/Datanexus/kafka) repository (the first option); making such changes will, more than likely, lead to conflicts at a later date when these files are modified in the main [kafka](https://github.com/Datanexus/kafka) repository in a way that is inconsistent with the values that you have set locally (in your clone of this repository).
